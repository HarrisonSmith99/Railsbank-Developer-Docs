...
```
### Endpoints
Base Endpoint: `GET /v1/customer/ledgers`
| Endpoint                                                                                              | Function |
|:------------------------------------------------------------------------------------------------------|:--|
| `/{{ledger_id}}`                                                                                      | Get a ledger based on its UUID. This will allow you to discover the account details that can be used in the following endpoints. |
| `/by-iban?iban={{YOUR_IBAN}}`                                                                         | Get a ledger based on its IBAN. |
| `/by-uk-bank-account?uk_account_number={{YOUR_UK_ACCOUNT_NUMBER}}&uk_sort_code={{YOUR_UK_SORT_CODE}}` | Get a ledger based on its uk account details. |
